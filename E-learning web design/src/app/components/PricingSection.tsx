...
const plans = [
  {
    id: "free", name: "Free", price: { m: 0, y: 0 }, desc: "Get started today",
    color: "#64748b",
    features: ["200+ free courses", "Community access", "Course previews", "Basic certificates", null, null],
    cta: "Start Free",
  },
  {
    id: "pro", name: "Pro", price: { m: 29, y: 19 }, desc: "Most popular choice",
    color: "#5eead4", popular: true,
    features: ["Unlimited courses", "Offline downloads", "2× mentor sessions/mo", "Pro certificates", "Learning paths", "Priority support"],
    cta: "Start Pro",
  },
  {
    id: "teams", name: "Teams", price: { m: 59, y: 39 }, desc: "For growing orgs",
    color: "#818cf8",
    features: ["Everything in Pro", "25 team seats", "Admin analytics", "Custom paths", "SSO & security", "Dedicated manager"],
    cta: "Start Trial",
  },
];
          ...
          <div className="inline-flex p-1 rounded-xl" style={{ background: "#0d1120", border: "1px solid rgba(94,234,212,0.08)" }}>
            {["Monthly", "Yearly"].map(o => (
              <button key={o} onClick={() => setYearly(o === "Yearly")}
                ...
                style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 600, fontSize: "0.85rem", border: "none", cursor: "pointer", background: (o === "Yearly") === yearly ? "#131829" : "transparent", color: (o === "Yearly") === yearly ? "#e2e8f0" : "#64748b" }}>
                {o}
                {o === "Yearly" && <span className="px-1.5 py-0.5 rounded" style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.58rem", background: "rgba(52,211,153,0.12)", color: "#34d399" }}>−34%</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {plans.map((plan, i) => (
            <div key={plan.id}
              ...
              {plan.popular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 flex items-center gap-1 px-3 py-1 rounded-full"
                  style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.65rem", background: "linear-gradient(135deg, #5eead4, #818cf8)", color: "#060812" }}>
              ...
              <div className="flex items-end gap-1 mb-7">
                ...
                {plan.price.m > 0 && (
                  <span style={{ fontFamily: "'DM Mono', monospace", color: "#64748b", fontSize: "0.78rem", paddingBottom: "6px" }}>/mo</span>
                )}
              </div>
